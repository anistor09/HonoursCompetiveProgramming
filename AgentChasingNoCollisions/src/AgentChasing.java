...
//            return 5;
    ...
    public int solveMinCostNoCol() {
        findDistances();
        int maxi = Integer.MIN_VALUE;
        for( int i = 0; i< distances.length; i++)
            for(int j = 0; j< distances[0].length; j++)
                maxi = Math.max(distances[i][j], maxi);
        int maxTimestamps = maxi + targetCount -1;
        MinCostMaxFlow mc = new MinCostMaxFlow();
        int res = mc.getMinCostNoCol(g,targetCount,vertexCount,startAgents,targetPaths, maxTimestamps);
